...
public class DimmableButtonPanel extends Button {
	...
	public DimmableButtonPanel(Context context) {
		
		super(context);
		PaddingLeft = (this.getPaddingLeft() <= 0) ? dpToPx(8) : this.getPaddingLeft();
		PaddingTop =  (this.getPaddingTop() <= 0) ? dpToPx(8) : this.getPaddingTop();
		PaddingRight =  (this.getPaddingRight() <= 0) ? dpToPx(8) : this.getPaddingRight();
		PaddingBottom =   (this.getPaddingBottom() <= 0) ? dpToPx(8) : this.getPaddingBottom();
		this.setPadding(PaddingLeft, PaddingTop, PaddingRight, PaddingBottom);
		init(null);
	}

	public DimmableButtonPanel(Context context, AttributeSet attrs, int defStyle) {
	...
	public DimmableButtonPanel(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs);
		PaddingLeft = (this.getPaddingLeft() <= 0) ? dpToPx(8) : this.getPaddingLeft();
		PaddingTop =  (this.getPaddingTop() <= 0) ? dpToPx(8) : this.getPaddingTop();
    ...
    public void resizeText(int width, int height) {
    	...
    	//if ((textWidth != 0.0)&((float)textWidth / (float)width) > 1)
    	//{
    ...
    private int getTextWidth(CharSequence source, TextPaint paint, int width, float textSize) {
    	...
    	if (source.toString().contains("\n"))
    	{
    		...
    		return  ((int) ((longest ) * textSize * 0.9));// + dpToPx(this.PaddingLeft) + dpToPx(this.PaddingRight);
    	}
    	
    	TextPaint paintCopy = new TextPaint(paint);
        // Update the text paint object
        paintCopy.setTextSize(textSize);
        return ((int) ((source.length()  ) * textSize * 1));// + dpToPx(this.PaddingLeft) + dpToPx(this.PaddingRight);
    }
}
